...
// // #define clogd(...) __android_log_print(ANDROID_LOG_INFO, "ccsIperf", __VA_ARGS__)
// #include <android/log.h>
#define LOG_TAG "ccsIperf"
...
{
    int i;
    struct iperf_stream *sp;//cczheng
    // char template[] = "/tmp/iperf3.XXXXXX";
    char template[] = "/sdcard/iperf3.XXXXXX";
    ...
    if (!sp->result) {
        free(sp);
        i_errno = IECREATESTREAM;
         ALOGD("%s:%s:%d\n","!sp->result",__FUNCTION__,__LINE__);
        ALOGD("%s","!sp->result");
        return NULL;
    }
    ...
    if (sp->buffer_fd == -1) {
        i_errno = IECREATESTREAM;
         ALOGD("%s:%s:%d\n","sp->buffer_fd == -1",__FUNCTION__,__LINE__);
        free(sp->result);
        free(sp);
        return NULL;
    }
    if (unlink(template) < 0) {
        i_errno = IECREATESTREAM;
         ALOGD("%s:%s:%d\n","unlink(template) < 0",__FUNCTION__,__LINE__);
        free(sp->result);
        free(sp);
        return NULL;
    }
    if (ftruncate(sp->buffer_fd, test->settings->blksize) < 0) {
        i_errno = IECREATESTREAM;
         ALOGD("%s:%s:%d\n","ftruncate",__FUNCTION__,__LINE__);
        free(sp->result);
        free(sp);
        return NULL;
    }
    ...
    if (sp->buffer == MAP_FAILED) {
        i_errno = IECREATESTREAM;
         ALOGD("%s:%s:%d\n","sp->buffer == MAP_FAILED",__FUNCTION__,__LINE__);
        free(sp->result);
        free(sp);
        return NULL;
    }
